...
export const createEntity = async ({ selectedNotes, scoreIri, projectIri, postAnnotation }) => {
  try {
    ...
    const offsets = !isGlobalAnnotation && selectedNotes.map(note => window.tk.getTimeForElement(note))
    const firstNote = !isGlobalAnnotation && selectedNotes[offsets.findIndex(e => e === Math.min(...offsets))]
    const page = isGlobalAnnotation ? 1 : window.tk.getPageWithElement(firstNote)
    ...
    const body = {
      ...
      document_context: scoreIri + '#' + page,
